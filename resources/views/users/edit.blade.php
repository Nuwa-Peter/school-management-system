        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('users.update', $user) }}" method="POST" x-data="{ role: '{{ old('role', $user->role->value) }}', selectedClass: '{{ old('class_level_id', $currentStream->class_level_id ?? '') }}', streams: {{ json_encode($streams) }} }">
                        @csrf
                        @method('PUT')
                                ...
                                <div>
                                    ...
                                    <select id="class_level_id" name="class_level_id" x-model="selectedClass" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                        ...
                                        @foreach($classLevels as $classLevel)
                                            <option value="{{ $classLevel->id }}" @selected(old('class_level_id', $currentStream->class_level_id ?? '') == $classLevel->id)>
                                                {{ $classLevel->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select id="stream_id" name="stream_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" :disabled="!selectedClass">
                                        ...
                                        <template x-for="stream in streams.filter(s => s.class_level_id == selectedClass)">
                                            <option :value="stream.id" x-text="stream.name" :selected="stream.id == {{ old('stream_id', $currentStream->id ?? 'null') }}"></option>
                                        </template>
                                    </select>
                                </div>
                            </div>
